...
void borrar_lista(Nodo* primero){
  Nodo *j=NULL;
...
int main(){
  ...
  struct s_autosalvado *s_save = NULL;
...
//Sentencias para controlar el menu
  do{
    ...
    switch(opcion)
    {
      ...
      case 10: //Ordenar Jedis por puntos de vida

        ordenar_jedi(primero);
        break;
      ...
      case 11: // Autosalvado
          ...
          if(guardar==0){
	           guardar = 1;
	           thr_rc = pthread_create(&thread, NULL, autosalvado, (void*)s_save);

          }else{
             printf("Error del hilo: %d\n",thr_rc);
          }
	     break;
...
void mostrar_info(Nodo *primero){
  if(primero==NULL){
    ...
    printf("Indique el ID: \n");
     ...
     for(j = primero; j != NULL; j = j->next){
       if(j->jedi.ID == id){
	  printf("Informacion completa de Jedi");
		printf("\n= id: %d",j->jedi.ID);
...
//Ordenar por puntos de vida (Metodo BuubbleSort)
...
void ordenar_jedi(Nodo *primero){
  ...
  if(primero == NULL){
    ...
    do{
      ...
      while(j->next != i){
        if(j->jedi.puntos.hit_points > j->next->jedi.puntos.hit_points){
          swap(j, j->next);
          swapped = 1;
        }
        j = j->next;
...
//Funcion auxiliar para intercambiar nodos al ordenar la lista

void swap(Nodo *a, Nodo *b){
  int temp = a->jedi.puntos.hit_points;
  a->jedi.puntos.hit_points = b->jedi.puntos.hit_points;
  b->jedi.puntos.hit_points = temp;
}
